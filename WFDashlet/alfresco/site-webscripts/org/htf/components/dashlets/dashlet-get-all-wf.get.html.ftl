...
<style>
	...
	#tbheader th,#maintable td {
	text-align: left;
	padding: 8px;
	}
	#tbheader tr:nth-child(even),#maintable tr:nth-child(even){background-color: #f2f2f2}
	#tbheader th,#maintable th {
...
<div style="display:none" id="apllyfiltergroup">true</div>
		...
		<table id="ghostlist" style="display:none">
			...
			<!--	<td>${task.currentTaskdescription}</td>-->
				...
				<td>${task.currentTaskPriority}</td>
			...
			if (tableghost.rows.length > 0){
				...
				for (var i = 0, row; row = tableghost.rows[i]; i++) {
					...
					if (ginit!=document.getElementById("selectfilter").innerHTML){						
						if (row.cells[7].innerHTML!=ginit)	
							show=false;								
					}	
					
					if (apllyfiltergroup=="true"){		
						if (row.cells[7].innerHTML!=groupthisuser)	
							show=false;								
					}
